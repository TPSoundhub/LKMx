...
#
# Knud - SHD - 14 sept. 2020 - Som ex opsamling efter observation i HH.
#
...
while freq_left < MAX_FREQ:
    ...
    #
    signal_left  = (np.sin(np.linspace(0,DURATION*freq_left*2*np.pi,DURATION*SAMPLERATE))*amp).astype(np.int16)
    signal_right = (np.sin(np.linspace(0,DURATION*freq_right*2*np.pi,DURATION*SAMPLERATE))*amp).astype(np.int16)
    ...
    else: pygame.time.delay(200)
    #
    # Med konstanterne kan man styre om der kun tegnes een kurve for den lyd der spiller eller om de tegnes oveni hinanden.
    # Nedenfor slettes gamle kurver hvis ONLY_ONE_CURVE er True. FFR får særstatus og kan styres individuelt.
    #
